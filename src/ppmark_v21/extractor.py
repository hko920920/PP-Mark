    ...
    def _extract_variant(self, image: np.ndarray, bits_needed: int) -> np.ndarray:
        ...
        for coeff in latent.coeffs:
            ...
            if np.isclose(low, high):
                bits_local = np.zeros_like(picked, dtype=np.uint8)
            else:
                dist_low = np.abs(picked - low)
                dist_high = np.abs(picked - high)
                bits_local = (dist_low <= dist_high).astype(np.uint8)
            votes.append(bits_local)
