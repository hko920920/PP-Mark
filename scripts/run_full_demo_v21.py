...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run provider setup, prover, and verifier in one shot (v2.1 semantic anchor).")
    parser.add_argument("--config", type=Path, default=Path("artifacts/provider_setup_v21.json"), help="Provider config JSON path.")
    ...
    parser.add_argument("--seed", type=str, required=True, help="Deterministic seed/nonce for the semantic anchor.")
    parser.add_argument("--model-id", type=str, required=True, help="Model identifier recorded in metadata.")
    parser.add_argument("--timestamp", type=str, required=True, help="Generation timestamp (ISO8601).")
    parser.add_argument("--parent-hash", type=str, help="Parent manifest hash (I2I provenance).")
    ...
    parser.add_argument("--signature-scheme", type=str, default="ecdsa-p256", help="Signature scheme for metadata.")
    return parser.parse_args()


def ensure_config(path: Path, halo2_dir: Path, base_image: Path, signature_scheme: str) -> ProviderConfig:
    if not path.exists():
        print(f"[demo] {path} missing — running Stage 1 setup.")
        create_provider_config(output_path=path, halo2_dir=halo2_dir, base_image=base_image, signature_scheme=signature_scheme)
    return ProviderConfig.load(path)


def main() -> None:
    args = parse_args()
    provider_cfg = ensure_config(args.config, args.halo2_dir, args.base_image, args.signature_scheme)
    ...
    outputs = prover.generate(
        ...
        parent_manifest_hash=args.parent_hash,
